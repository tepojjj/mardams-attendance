...
// validate them. Only ever called for Super Admin requests — this is still
// configured from the Job Orders app's Users tab; this app only reads it.
...
// cashAdvanceDeduction off a request body and validate them. Unlike
// payType/rate/shift (Super-Admin-only, set from Job Orders), these are the
// figures Accounting actually runs payroll with, so both Accounting and the
// Super Admin can set them here, from the Payroll tab. Returns { fields, error }.
const MONEY_FIELDS = [
...
module.exports = async (req, res) => {
  ...
  if (req.method === 'GET') {
    const auth = requireRole(req, res, ['admin', 'super_admin', 'accounting']);
    if (!auth) return;

    const users = (await kv.hgetall(USERS_KEY)) || {};
    const canSeePayroll = auth.role === 'super_admin' || auth.role === 'accounting';
  ...
  //  - Payroll shift configuration (pay type, rate, regular shift times) —
  //    Super Admin only, set from the Job Orders app.
  //  - Payroll money fields (OT rate, allowance, SSS/PhilHealth/Pag-IBIG/
  //    Cash Advance deductions) — Accounting or Super Admin, set from this
  //    app's Payroll tab.
  if (req.method === 'PATCH') {
    const auth = requireRole(req, res, ['admin', 'super_admin', 'accounting']);
    if (!auth) return;
    ...
    if (body.newPassword !== undefined) {
      if (auth.role === 'accounting') {
        res.status(403).json({ error: 'Accounting accounts cannot reset passwords' });
        return;
      }
    ...
    // Pay type / rate / regular shift times — Super Admin only. (Not
    // actually edited from this app's UI, which has no Users tab, but the
    // check stays here in case of a direct API call.)
    if (auth.role === 'super_admin') {
      const { fields, error } = parsePayrollFields(body);
      if (error) {
        res.status(400).json({ error });
      ...
      if (Object.keys(fields).length) {
        res.status(403).json({ error: 'Only the Super Admin can edit pay type, rate, or shift times' });
        return;
      }
    }

    // OT rate / allowance / deductions — Accounting or Super Admin, from
    // this app's Payroll tab.
    if (auth.role === 'super_admin' || auth.role === 'accounting') {
      ...
      if (Object.keys(fields).length) {
        res.status(403).json({ error: 'Only Accounting or the Super Admin can edit payroll deductions' });
        return;
      }
    }

    if (auth.role === 'accounting' && body.department !== undefined) {
      res.status(403).json({ error: 'Accounting accounts cannot edit department' });
      return;
    }
    const { fields: deptFields, error: deptError } = parseDepartmentField(body);
    if (deptError) {
